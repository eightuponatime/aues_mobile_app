                                ...
                                Button(
                                    onClick = {
                                        ...
                                        CoroutineScope(Dispatchers.IO).launch {
                                            password = getPassword(user_username)

                                            try {
                                                result = scheduleParser(user_username, password)

                                                if (result.startsWith("День недели: Понедельник")) {
                                                    val scheduleDao = DatabaseManager.getDatabase().scheduleDao()

                                                    val existingSchedule = scheduleDao.getScheduleByUsername(user_username)

                                                    if (existingSchedule != null) {
                                                        existingSchedule.schedule = result
                                                        scheduleDao.update(existingSchedule)
                                                    } else {
                                                        val scheduleEntity = ScheduleEntity(username = user_username, schedule = result)
